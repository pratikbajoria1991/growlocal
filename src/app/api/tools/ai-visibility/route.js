...
import { checkAiVisibility } from "@/lib/ai-visibility";
...
export async function POST(req) {
  ...
  try {
    return NextResponse.json(await checkAiVisibility(url));
  } catch (e) {
    return NextResponse.json({ error: e.message || "Check failed." }, { status: 502 });
  }
}
